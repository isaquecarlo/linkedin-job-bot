...
def load_config():
    ...
    if not config_path.exists():
        raise FileNotFoundError("config.json não encontrado! Use config.json.example como template.")
...
CV_PT = Path(__file__).parent / CONFIG["pdf_paths"]["cv_pt"]
CV_EN = Path(__file__).parent / CONFIG["pdf_paths"]["cv_en"]
CL_PT = Path(__file__).parent / CONFIG["pdf_paths"]["cl_pt"]
CL_EN = Path(__file__).parent / CONFIG["pdf_paths"]["cl_en"]

SALARIO_NUMERO = CONFIG["salary_number"]
...
def get_strategic_answer(question_text, is_numeric_field=False, is_english=False, options=None):
    """Cérebro estratégico: Gera respostas inteligentes para perguntas de candidatura"""
...
# --- REGRAS DINÂMICAS DA IA (Do config.json) ---
try:
...
1. **AUTHORIZATION & VISA (AGGRESSIVE STRATEGY)**:
   - If asked "Are you legally authorized to work in [Country]?", ALWAYS answer "{AI_RULES.get('authorized', 'Yes')}".
   - If asked "Will you now or in the future require sponsorship?", answer "{AI_RULES.get('sponsorship', 'No')}" (Assume B2B/Contractor contract which handles this).
...
2. **SALARY & CURRENCY (CONTEXT AWARE)**:
   - IF the Job Description is in ENGLISH: Assume the currency is USD or EUR.
     - Expected Salary: "{AI_RULES.get('salary_usd', '3000')}" (Do NOT put {AI_RULES.get('salary_brl', '3500')} BRL, that is too low for USD).
   - IF the Job Description is in PORTUGUESE: Assume BRL.
     - Expected Salary: "{AI_RULES.get('salary_brl', '3500')}".
   - If the question asks for a generic number without currency specified, look at the language.

3. **EXPERIENCE (NUMERIC)**:
   - If asked "How many years...", answer "{AI_RULES.get('experience_years_python', '5')}" for Python/Data/AI roles.
   - For Cloud/Azure, answer "{AI_RULES.get('experience_years_cloud', '3')}".
   - NEVER answer "Yes" to a numeric question.

4. **LOCATION**:
   - If asked "Are you comfortable working remotely?", answer "{AI_RULES.get('remote_work', 'Yes')}".
   - If asked "Where do you live?", answer "{AI_RULES.get('city_country', 'Florianópolis, Brazil')}".
"""
...
def fill_form_turbo(page, candidate, is_english):
    """Preenche o formulário ativo de candidatura do LinkedIn"""
    print(f"      🚀 Preenchendo formulário... is_english={is_english}")
...
# Verifica se já está preenchido
            ...
            # 3. É Input Texto/Numérico
                ...
                # --- DETECTOR DE RANGE DE VALIDAÇÃO ---
                # Lê mensagens de erro para detectar ranges permitidos (ex: "válida (1-2)")
                detected_range = None
                try:
                    error_msg = block.locator(".artdeco-inline-feedback--error").first.inner_text()
                    # Procura padrões como (1-2), (0-10), (1-5)
                    import re
                    range_match = re.search(r'\((\d+)-(\d+)\)', error_msg)
                    if range_match:
                        min_val = int(range_match.group(1))
                        max_val = int(range_match.group(2))
                        detected_range = (min_val, max_val)
                        print(f"         🎯 Range detectado: {min_val}-{max_val}")
                except:
                    pass
                
                answer = get_strategic_answer(lbl, is_numeric, is_english)
                
                # Ajusta resposta se estiver fora do range detectado
                if detected_range and is_numeric:
                    try:
                        ans_num = int(answer)
                        min_val, max_val = detected_range
                        if ans_num < min_val or ans_num > max_val:
                            # Ajusta para o valor máximo do range (mais otimista)
                            answer = str(max_val)
                            print(f"         ⚙️ Resposta ajustada para range: {answer}")
                    except:
                        pass
...
def run_bot():
    print("🚀 LinkedIn Job Application Bot - Iniciando...")
    config_path = Path(__file__).parent / "config.json"
                    ...
                    try:
                        ...
                        for i, card in enumerate(job_cards[:25]):
                            ...
                            try:
                                ...
                                # COTA ESPANHOL (5/dia)
                                # FILTRO ESPANHOL: O usuário pediu para NÃO aplicar
                                if lang_code == "es":
                                    print("      🛑 Vaga em Espanhol detectada. Ignorando (conforme configuração).")
                                    continue
